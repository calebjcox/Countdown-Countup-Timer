...
object WidgetPalette {
     ...
     * the pair rather than another qualified resource is the opacity — a number chosen
    ...
    fun backgroundFor(context: Context, backdrop: Backdrop, textTheme: TextTheme): Int {
        ...
        // the tint beside it, which is the only thing that can carry an opacity. See
    ...
    fun scrimTintFor(
        ...
        val named = textTheme.chosenTone
            ?: return ScrimTint(
                notNight = wash(context, R.color.widget_scrim_light, scrimOpacity),
                night = wash(context, R.color.widget_scrim_dark, scrimOpacity),
            )
        val hue = if (named.isLight) R.color.widget_scrim_dark else R.color.widget_scrim_light
        val fixed = wash(context, hue, scrimOpacity)
    ...
    fun surfaceColorFor(
        ...
        // for.
        scrimTintFor(context, backdrop, textTheme, scrimOpacity)?.let { tint ->
    ...
    /**
     * One of the two hues at the opacity a timer asked for.
     *
     * The hues are opaque, so the alpha is entirely the dial's — which is what makes it
     * mean what it says, and why nothing may put an alpha back into the resources. See
     * colors.xml.
     */
    @ColorInt
    private fun wash(context: Context, @ColorRes hue: Int, scrimOpacity: Int): Int =
        ColorUtils.setAlphaComponent(
            color(context, hue),
            Math.round(ScrimOpacity.snap(scrimOpacity) * 255f / ScrimOpacity.MAX),
        )
