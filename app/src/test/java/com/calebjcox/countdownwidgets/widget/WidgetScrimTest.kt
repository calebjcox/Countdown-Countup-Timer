 ...
 * test that read them together could pass on a wash going the right way at an opacity
 * nobody asked for.
 */
...
class WidgetScrimTest {
    ...
    @Test
    fun `the opacity dial moves the wash's alpha and nothing else`() {
        // What the dial is allowed to be: how much of one of two fixed hues is drawn. An
        // opacity that reached the hue would be a second way to choose the tone, running
        // against the tone the text already chose.
        val opacities = listOf(ScrimOpacity.MIN, 40, ScrimOpacity.DEFAULT, ScrimOpacity.MAX)
        val washes = opacities.map { opacity ->
            washOf(
                timer.copy(
        ...
        assertEquals(
            "the dial changed which wash it was, not how much of it",
            listOf(ContextCompat.getColor(context, R.color.widget_scrim_dark)),
            washes.map(::hueOf).distinct(),
        )
        // Both ends land where they say they do rather than near it, which is the half of
        // the dial a rounding error hides in: nothing on screen distinguishes 98% from
        // solid, and the widget at either end is one the user has to be able to get back.
        assertEquals("the bottom of the dial drew some wash", 0, Color.alpha(washes.first()))
        assertEquals(
            "the top of the dial drew less than a solid wash",
            255,
            Color.alpha(washes.last()),
        )
        for ((thinner, thicker) in washes.zipWithNext()) {
            assertTrue(
                "a higher opacity did not draw more of the wash",
                Color.alpha(thicker) > Color.alpha(thinner),
            )
        }
    }
    ...
    fun `the other two backdrops draw no scrim`() {
        ...
        // half of that the dial could leak through: a panel at a low opacity is a panel
        // nobody can read.
        ...
        for (backdrop in listOf(Backdrop.NONE, Backdrop.PANEL)) {
            assertNull(
                "$backdrop was tinted by an opacity that is not its business",
                washOf(timer.copy(backdrop = backdrop, scrimOpacity = ScrimOpacity.MIN)),
            )
        }
    }
    ...
    /** A colour with its alpha put back to solid, so two opacities of one hue compare equal. */
